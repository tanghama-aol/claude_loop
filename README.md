...
任务页为左树右编辑布局：左侧是项目任务树，点击任务即在右侧编辑该任务，标题、任务需求、所属项目、生成 Profile、执行 Profiles 和媒体产物设置修改后约 0.6 秒自动保存（`PATCH /api/tasks/:id`），状态条显示「保存中」「已自动保存 时间」或失败原因。目标文件名、工作目录、任务类型和任务来源在创建后锁定，正文请在运行页或编辑页修改。运行、排队、预约、等待重试或已归档的任务只读。Agent 生成模式下目标文件仍是占位内容时，改标题或需求会同步重写占位文件。点击「新建任务」回到创建表单；树中的「日志」按钮仍直接跳到运行页。
